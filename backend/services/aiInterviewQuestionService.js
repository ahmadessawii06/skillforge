...
const ALLOWED_TYPES = new Set(['technical', 'behavioral', 'problem_solving', 'system_design', 'communication']);
...
async function generateInterviewQuestions(input = {}) {
  ...
  try {
    ...
    return normalizeGeneratedQuestions(parseJsonResponse(aiResponse), request.count);
  } catch (error) {
    ...
    if (error.status === 401) throw new Error('Invalid NVIDIA API configuration');
    if (error.status === 404) throw new Error(`Model not found or unavailable: ${AI_CONFIG.model}`);
    if (error.status === 410) throw new Error(`Model was deprecated/removed by NVIDIA: ${AI_CONFIG.model}`);
...
function normalizeRequest(input) {
  ...
  const difficulty = ALLOWED_DIFFICULTIES.has(String(input.difficulty).toLowerCase())
  ...
  return {
    role: String(input.role || input.targetRole || 'Frontend Developer').trim().slice(0, 80),
    experienceLevel: String(input.experienceLevel || 'junior').trim().slice(0, 40),
    difficulty,
    count,
    skills: normalizeStringList(input.skills, 12),
    questionTypes,
    cvSummary: input.cvSummary ? String(input.cvSummary).trim().slice(0, 1200) : ''
  };
}
...
function buildUserPrompt(request) {
  const skills = request.skills.length ? request.skills.join(', ') : 'general software engineering skills';
...
function normalizeQuestion(question, index) {
  ...
  const questionText = String(question.questionText || question.question_text || '').trim().slice(0, 220);
  if (!questionText) return null;

  const questionType = ALLOWED_TYPES.has(String(question.questionType).toLowerCase())
    ? String(question.questionType).toLowerCase()
    : 'technical';

  return {
    id: index + 1,
    title: String(question.title || toTitle(questionType)).trim().slice(0, 60),
    questionText,
    questionType,
    questionOrder: index + 1,
    difficulty: normalizeDifficulty(question.difficulty),
    tags: normalizeStringList(question.tags, 4),
    hint: String(question.hint || '').trim().slice(0, 220),
    explanation: String(question.explanation || '').trim().slice(0, 400),
    options
  };
}
...
function normalizeOptions(options) {
  ...
  const parsed = options.slice(0, 4).map(option => {
    ...
    return {
      text: String(option.text || option.option_text || '').trim().slice(0, 120),
